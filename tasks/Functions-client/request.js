...
task("functions-request", "Initiates a request from a Functions client contract")
  ...
  .addParam("propid", "Billing subscription ID used to pay for the request")
  .addOptionalParam(
    "simulate",
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const proposalId = BigInt(taskArgs.propid);
    if (gasLimit > 300000) {
      throw Error("Gas limit must be less than or equal to 300,000")
    }

    // Attach to the required contracts
    const clientContractFactory = await ethers.getContractFactory("GovernorContract")
    ...
    // const proposalId = BigInt("77414520699582466147732147435107753430005427097115474008413754380562827677732");
