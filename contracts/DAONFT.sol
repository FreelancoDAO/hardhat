...
contract DaoNFT is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
  ...
  constructor(
    ...
    i_mintFee = 100000000000000000;
    i_callbackGasLimit = callbackGasLimit;
    _initializeContract(level_0_dogTokenUris, level_1_dogTokenUris, level_2_dogTokenUris);
    s_tokenCounter = 0;
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
    ...
    string memory _uri = getRandomNFT(moddedRng, s_requestIdToLevel[requestId]);
  ...
  function getRandomNFT(uint256 moddedRng, Level breed) public view returns (string memory) {
  ...
  function withdraw() public onlyOwner {
    ...
    address[] memory holders = repoContract.getHolders();
    for (uint256 i = 0; i < holders.length; i++) {
      address holder = holders[i];
      uint8 level = repoContract.getRepo(holder);
      if (level == 1) {
        totalSoldiers++;
      } else if (level == 2) {
        totalMarines++;
  ...
  function getDogTokenUris(uint256 index, Level level) public view returns (string memory) {
